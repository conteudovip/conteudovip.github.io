...
const TELEGRAM_BOT_USERNAME = window.__TELEGRAM_BOT_USERNAME__ || "";
const USE_TELEGRAM_BOT = window.__USE_TELEGRAM_BOT__ || false;
...
const openCheckout = (product) => {
  ...
  // Se estiver usando modo bot-only (GitHub Pages), redireciona para o bot
  if (USE_TELEGRAM_BOT && TELEGRAM_BOT_USERNAME) {
    const botUrl = `https://t.me/${TELEGRAM_BOT_USERNAME}?start=pix-${product.product_id}`;
...
const bootstrap = async () => {
  ...
  try {
    ...
    // Modo bot-only: carrega do JSON estático
    if (USE_TELEGRAM_BOT) {
      console.log("🔍 Carregando produtos do arquivo JSON estático...");
      const response = await fetch("./products.json");
      
      if (!response.ok) {
        throw new Error(`Erro ao carregar products.json: ${response.status}`);
      }
      
      data = await response.json();
      console.log("✅ Produtos carregados do JSON estático:", data);
    } else {
      // Modo normal: carrega da API
    ...
    const errorMsg = USE_TELEGRAM_BOT
      ? `Erro ao carregar products.json: ${error.message}\n\nExecute: python gerar-produtos-json.py`
